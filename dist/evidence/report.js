 ...
 * 生成内容到证据的映射。
 */
export function buildContentEvidenceMap(generatedReadme, facts) {
    const sections = [];
...
function findEvidenceRefs(content, items) {
    return items
        .map((item, idx) => (content.includes(item.content.slice(0, 30)) ? idx : -1))
        .filter((idx) => idx >= 0);
}
function hasSupportingEvidence(content, items) {
    return findEvidenceRefs(content, items).length > 0;
}
/**
 * 格式化证据报告，用于终端输出。
 */
export function formatEvidenceReport(facts, contentEvidenceMap) {
    const lines = [];
    ...
    for (const [category, items] of byCategory) {
        ...
        for (const item of items) {
            ...
            for (const ev of item.evidence) {
                let loc = ev.filePath;
                if (ev.lineRange)
                    loc += `:${ev.lineRange[0]}-${ev.lineRange[1]}`;
                if (ev.symbol)
                    loc += ` → ${ev.symbol}`;
                lines.push(`    [${ev.sourceType}] ${loc} (${ev.extractionMethod}, ${(ev.confidence * 100).toFixed(0)}%)`);
            }
            lines.push("");
        }
    }
    ...
    for (const section of contentEvidenceMap) {
        ...
        lines.push(`  ${status} ${section.section}: ${section.evidenceRefs.length} 项证据`);
    }
    lines.push("");
    // 无证据章节警告
