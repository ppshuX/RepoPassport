 ...
 */
export async function createDraftPr(owner, repo, baseBranch, headUser, headBranch, title, body, log) {
 ...
 * 查询 PR 状态。
 */
export async function viewPr(prUrl, log) {
    log.verbose(`查询 PR 状态: ${prUrl}`);
    const stdout = execSync(`gh pr view "${prUrl}" --json state,mergedAt,closedAt`, { encoding: "utf-8", stdio: "pipe" }).trim();
