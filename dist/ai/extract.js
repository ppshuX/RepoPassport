import { validateRepoFacts, buildPartialRepoFacts, FactsItemSchema, } from "../validate/facts.js";
/**
 * 第一步：从仓库文件中提取结构化 RepoFacts。
 */
export async function extractFacts(files, repoMeta, provider, log) {
    log.info("提取仓库事实...");
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
        try {
            ...
            if (result.valid && result.data) {
                ...
                log.info(`事实提取完成: ${facts.items.length} 项，总置信度 ${facts.overallConfidence}`);
                return facts;
            }
            // 校验不通过，尝试用部分有效项构建
            if (result.validItems !== undefined && result.validItems !== null) {
                log.warn(`Zod 校验: ${result.totalItems} 项中 ${result.validItems} 项有效`);
                // 尝试从 raw 中获取 items
                const rawObj = raw;
                ...
                if (attempt < maxRetries - 1) {
                    log.warn("有效项不足阈值，重试...");
                    lastError = new Error(`有效事实项不足: ${result.validItems} < 3`);
                    continue;
                }
            }
            if (attempt < maxRetries - 1) {
...
function buildExtractUserPrompt(files, repoMeta) {
