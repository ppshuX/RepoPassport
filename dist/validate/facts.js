 ...
 */
export function validateRepoFacts(raw) {
 ...
 */
export function buildPartialRepoFacts(repoMeta, validItems, minItems = 3) {
    if (validItems.length < minItems)
        return null;
    const overallConfidence = validItems.reduce((sum, item) => sum + item.combinedConfidence, 0) / validItems.length;
    return {
        repo: repoMeta,
